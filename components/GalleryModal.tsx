...
const GalleryModal: React.FC = () => {
    ...
    return (
        <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-8 animate-fade-in">
            <div className="bg-paper-100 w-full max-w-4xl max-h-[90vh] rounded-lg flex flex-col overflow-hidden border-4 border-gold-600 shadow-2xl">
                <div className="bg-ink-900 p-4 flex justify-between items-center border-b border-gold-600">
                    <h2 className="text-gold-500 font-display text-2xl flex items-center gap-2">
                        <LucideImage /> The Sketchbook of Memory
                    </h2>
                    <button onClick={() => dispatch({ type: 'CLOSE_GALLERY' })} className="text-paper-100 hover:text-red-400">
                        <LucideX size={24} />
                    </button>
                </div>
                
                <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 md:grid-cols-2 gap-8 bg-paper-200">
